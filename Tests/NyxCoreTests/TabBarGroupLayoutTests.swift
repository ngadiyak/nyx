...
@Test func anExpandedGroupKeepsASlotPerTab() {
    let g = grouped()
    let slots = TabBarGeometry.slots(tabCount: 5, grouping: g)
    #expect(slots.count == 5)
    #expect(slots[1] == .tab(index: 1, group: groupID(g)))
    #expect(slots[3] == .tab(index: 3, group: nil))
}
...
/// A group's name is shown above its tabs, which needs a row the bar does not otherwise have. An
/// ungrouped window keeps exactly the bar it always had.
@Test func theBarGrowsOnlyWhenAGroupIsExpanded() {
    #expect(TabBarGeometry.barHeight(base: 28, grouping: TabGrouping(tabCount: 3)) == 28)
    #expect(TabBarGeometry.barHeight(base: 28, grouping: grouped()) == 28 + metrics.groupHeaderHeight)
    #expect(TabBarGeometry.barHeight(base: 28, grouping: grouped(collapsed: true)) == 28)
}

// MARK: - Where things sit

/// The header row is taken off the top, so a group's name never overlaps the tab it names.
@Test func slotsSitBelowTheHeaderRow() {
    let slot = TabBarGeometry.slotRect(index: 0, slotCount: 4, barWidth: 400, barHeight: 42,
                                       headerHeight: 14)
    #expect(slot.y == 14)
    #expect(slot.height == 28)
}

@Test func aHeaderSpansExactlyItsGroupsTabs() {
    let slots = TabBarGeometry.slots(tabCount: 5, grouping: grouped())
    let headers = TabBarGeometry.groupHeaders(slots: slots)
    #expect(headers.count == 1)
    #expect(headers[0].first == 1)
    #expect(headers[0].last == 2)

    let rect = TabBarGeometry.groupHeaderRect(fromSlot: 1, toSlot: 2, slotCount: 5,
                                              barWidth: 500, headerHeight: 14)
    #expect(rect.x == 100)
...
@Test func theHeaderRowOverAnUngroupedTabMeansNothing() {
    let slots = TabBarGeometry.slots(tabCount: 5, grouping: grouped())
    let hit = TabBarGeometry.hit(atX: 50, y: 5, slots: slots, barWidth: 500, barHeight: 42,
                                 headerHeight: 14)
    #expect(hit == nil)
}

/// A click below the header row is still a click on the tab, and it means the tab's own index --
/// not its position in the bar, which a collapsed group to its left has shifted.
@Test func aClickOnAGroupedTabSelectsThatTab() {
    let slots = TabBarGeometry.slots(tabCount: 5, grouping: grouped())
    let hit = TabBarGeometry.hit(atX: 150, y: 30, slots: slots, barWidth: 500, barHeight: 42,
                                 headerHeight: 14)
    #expect(hit == .select(1))
}
