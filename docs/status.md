...
| Remote sessions | A paired Mac's shell as a tab: relay-based, end-to-end encrypted (X25519 per attach, signed by an Ed25519 device identity, ChaCha20-Poly1305 with a per-direction counter), six-character pairing code with a four-word fingerprint both people compare, palette Remote section, snapshot-then-live with the host's OSC 133 marks intact, one writer with take-control, an audit log on the host. A dropped socket resumes the attachment where it left off (a full re-snapshot when the host cannot prove what was delivered), the mirror follows the host's size, a Mac that unpaired you says so (`not_paired`) instead of looking asleep, a forgotten Mac comes back under its declared name, the palette's relay row opens the Remote page, a pasted token survives pressing Pair, and the strip's Take control is a drawn button with a 3:1 bezel. The pairing sheet is one sentence and a spinner with Return on its primary button, the Remote page says what a remote session is and why Pair is dead right beside the button (and wakes it as you type the token), and a right-click anywhere on the tab bar that is not a tab offers `New Remote Tab…` plus a one-line row per online session; a tab's own menu carries that tail only when remote sessions are on. Verified with two instances against the deployed relay, one attachment held 55 minutes without a drop |
...
| Verification | 2229 swift-testing tests, offscreen Metal pixel tests, UI snapshot renderer, an opt-in run against a real relay binary (`NYX_RELAY_BIN`), CI with warning gate, bench floor, launch check and snapshot artifact |
